...
public:
    ...
    void reverseTraversal(Node* node, std::ofstream& outfile)
    {
        if (node == nullptr) { return; }

        reverseTraversal(node->left, outfile);
        reverseTraversal(node->right, outfile);
        outfile << node->data << " ";
    }
    ...
    {
        directTraversal(root, outfile);
        outfile << std::endl;
        reverseTraversal(root, outfile);
    }
};
